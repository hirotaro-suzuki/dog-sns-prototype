...
- 開始日、終了日の入力欄は、iPad Safariの標準日付入力による曜日括弧表示を避けるため、通常の `YYYY-MM-DD` 入力へ補正する `AdminDateInputNormalizer` を追加した。
...
- GitHub main: 開始日、終了日の括弧表示対策として、日付入力を `YYYY-MM-DD` の通常入力へ補正する変更を反映済み。
...
- 開始日、終了日の入力欄は、iPad Safariの標準日付入力が曜日を括弧つきで表示することがあるため、`AdminDateInputNormalizer` で通常の `YYYY-MM-DD` 入力へ補正する。
