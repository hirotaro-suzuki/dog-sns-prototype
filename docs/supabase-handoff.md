...
- 写真を完全に削除する（顧客からの申し入れなど必要な場合。Storage画像とDBレコードを両方削除、2026-07-09）
- 店舗マスタの店舗名、表示名、並び順、有効/無効、メモを編集する（担当者マスタ編集も同じ「店舗」タブ内、2026-07-09に統合）
- 担当者マスタの追加、担当者名、有効/無効、並び順を編集する（役割・SNS承認可否は2026-07-09に廃止）
- 店舗ごとの写真枠（最大3枚）をサムネイルで追加、差し替え、並び替え、削除で管理する（無効化ではなく削除、2026-07-08）
...
- 開始日、終了日は `YYYY-MM-DD` の文字入力欄と「カレンダー」ボタン（端末標準の日付選択を開く）で指定する（iPad Safariの標準日付入力の表示乱れを避けるため。2026-07-13に実装を整理）

SNS投稿文の作成と保存、SNS自動投稿、Instagram連携は次フェーズ以降で検討する。
店舗ログインコードとPINハッシュの画面編集はまだ対象外とする（写真の完全削除は2026-07-09に実装済み。2026-07-13からは、Storage削除が失敗した場合は完全削除を中止してエラーを表示する）。
