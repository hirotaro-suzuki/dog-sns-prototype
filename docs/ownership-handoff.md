...
- 写真タブの複数選択UIを含むコミット `82a4609c3246bf625c0ac1c56c71ee16401d2e96` はVercel status `success`。
- Vercel上で `/admin` 写真タブの実操作確認はまだ未確認。
- iPad Safariでの `/admin` 写真タブ確認はまだ未確認。
- 日付の後ろに括弧が残って見える場合、GitHub main上の最新コードではなく、古いVercel表示、ブラウザキャッシュ、または別箇所の表示を見ている可能性がある。
