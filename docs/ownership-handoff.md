...
- 保存済み写真を、今日、日付範囲、店舗、確認状態で絞り込める。
- 保存済み写真を、日付順の新しい順/古い順、または店順で表示できる。
...
- 写真カード上に確認状態と一言メモを表示できる。
- 写真タブはサムネイル一覧画面と写真詳細画面に分離済み。
- 写真一覧でチェックボックスによる複数選択ができる。
- 複数選択した写真を、詳細画面で `前へ`、`次へ`、`保存して次へ` により順番に確認できる。
- 写真詳細側で確認状態と一言メモを保存できる。
- 写真詳細側には旧来の `説明文` 欄を出さない。
- 写真カード、詳細、開始日、終了日の不要な括弧表示を出さない。
...
- 写真タブの複数選択UIと開始日・終了日の括弧表示対策はGitHub mainへ反映済みで、Vercel status は `success`。
- ユーザー確認で、写真一覧の選択用チェックボックスは画面に出ており、動きも確認済み。
- ユーザー確認で、開始日・終了日の後ろに出ていた括弧表示は消えた。
- 40文字超過の一言メモがAPIで拒否されることの実操作確認はまだ未確認。
- 店順が実データ上で `stores.sort_order` 優先になっていることの実操作確認はまだ未確認。
