...
- `docs/handoff/`（01〜07）
  - 引き継ぎ相手（オーナー・技術担当・店舗/本部の運用担当）向けの、読み手別の平易な引き継ぎ資料一式。2026-07-20作成開始。
  - 01 はじめに読む紙、02 しくみの図解、03 店舗マニュアル、04 本部マニュアル、05 困ったときの手引き（作成中）、06 権利・アカウント引き継ぎ（平易版。技術的な正本は `ownership-handoff.md`）、07 技術引き継ぎの入口。
...
- `docs/audit-2026-07-13.md`
  - 2026-07-11監査（GPT5.6）をClaude Fable 5が1件ずつ裏取りし、新規指摘を加えた再監査記録。
  - **次回の相談では最初に読む。** 引き継ぎ前の整理を1件ずつ決めるためのベース文書で、各項目に「判断」欄がある。

- `docs/audit-2026-07-11.md`
  - GitHub mainを基準に、コード、DB定義、全migration、文書、店舗側UI、管理側UIを再監査した記録（GPT5.6）。
  - 店舗保存APIは現場優先で現状維持、Admin PIN強化は引き渡し後の候補、という2026-07-11の判断も記録している。
  - 相談のベースは `docs/audit-2026-07-13.md` を優先する。
