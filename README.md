...
このプロジェクトは、ある程度使える形になったら店舗側または運用主体へ引き渡す前提です。そのため、特定のPCやDropboxに依存せず、GitHub、Vercel、Supabaseを見れば次の人が続けられる状態を重視します。
...
- 完成画像を生成し、ブラウザ標準の印刷画面を開ける
- SNS掲載OKをもらった完成画像をSupabase StorageとDBへ保存できる
- 本部用 `/admin` で保存済み写真を一覧確認、複数選択、順番確認できる
- 本部用 `/admin` で保存済み写真の一言メモと確認状態を更新し、非表示/復帰できる
- 本部用 `/admin` で店舗マスタ、担当者マスタ、写真枠を管理できる

詳しい仕様は `dog_sns_design.md` を確認します。

## 当面の優先事項

1. `/admin` 写真タブ、店舗タブ、担当者タブ、枠タブの安定確認
2. 店舗側の撮影、編集、印刷、同意後保存の流れの安定確認
3. 残っているロゴ管理を削除または非表示にするか判断
4. 正式ロゴ入りL版フレームの作成と確認
