...
店舗分散型SNS投稿支援システムのプロトタイプです（SNSへの自動投稿機能はありません。自動投稿は後続フェーズの構想です）。

友人が経営する飲食店向けに、店頭のiPadで「今日のわんちゃん」写真を撮影、加工し（希望があれば印刷し）、SNS掲載OKをもらった完成画像だけをクラウドへ保存する仕組みを作っています。
...
- 撮影担当者を選んでから、iPad Safariで最大3枚まで撮影できる（担当者を選ぶまで撮影できない）
- 撮影画像を長辺2400pxまでに軽量化し、クラウドへ送らず一時保持できる
- 3枚から1枚を選べる
- 写真を移動、拡大縮小、回転できる
- 必要な場所だけ手動モザイクできる
- 写真上へ短い文字ボックスを追加、編集、削除できる
- 正方形の完成画像を生成し、希望があればブラウザ標準の印刷画面を開ける（印刷は任意で、保存の前提条件ではない）
...
1. 引き継ぎ資料一式 `docs/handoff/` の完成と、引き継ぎ相手への説明
2. `/admin` 各タブ、写真の完全削除、枠寸法チェックの実画面確認
3. 本番切替（デモデータ削除→本番店舗・担当者・枠の投入）
4. アカウント・契約の移管（`docs/handoff/06-ownership-transfer.md`）
...
- `docs/handoff/`: 引き継ぎ相手（オーナー・技術担当・店舗/本部の運用担当）向けの平易な引き継ぎ資料一式
